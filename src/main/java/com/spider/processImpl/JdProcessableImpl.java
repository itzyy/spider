 ...
 */
public class JdProcessableImpl implements Processable<JdPage> {
    ...
    public void process(JdPage jdPage) {
        ...
        //将页面封装成tagnode对象
        TagNode rootNode = htmlCleaner.clean(jdPage.getContent());

        //判断是否是列表还是商品明细界面
        if (jdPage.getUrl().startsWith("http://list.jd.com/list.html")) {
            ...
            if (!href.equals("javascript:;")) {
                jdPage.addUrl("http://list.jd.com" + href);
            }
            //下一页地址
//            System.out.println("http://list.jd.com"+href);
            ...
            try {
                ...
                for (Object obj : urlList) {
                    ...
                    jdPage.addUrl("http:" + urlNode.getAttributeByName("href"));
//                    System.out.println("http:"+urlNode.getAttributeByName("href"));
            ...
            //解析商品明细界面
            parse(rootNode, jdPage);
        }
    }

    private void parse(TagNode rootNode, JdPage jdPage) {
        //根据xpath，进行快速查找
        // 两个//代表获取所有标签
        try {
            //获取标题
            String title = HtmlUtil.getText(rootNode, "//div[@class=\"sku-name\"]");
            jdPage.addField("title", title);
            //获取图片
            String imgPath = HtmlUtil.getText(rootNode, "//img[@id='spec-img']");
            jdPage.addField("imaPath", "http" + imgPath);
            //获取价格,由于价格是异步加载的，所以需要分析js请求
            getPrice(jdPage);
            //获取规格参数
            getGuiGe(jdPage, rootNode);
        } catch (XPatherException e) {
            e.printStackTrace();
        }
    }

    private void getGuiGe(JdPage jdPage, TagNode rootNode) throws XPatherException {
...
//        System.out.println(specArray.toString());
        jdPage.addField("spec", specArray.toString());
    }

    private void getPrice(JdPage jdPage) {
        String url = jdPage.getUrl();
        ...
        if (matcher.find()) {
            ...
            //设置编号
            jdPage.setProductNo(RevUtils.reverse(proId) + "_jd");
